...
def prior_transform_NFW(theta):
    ...
    return  np.array([10, 10E10]) * theta
...
def prior_transform_Einasto(theta):
    ...
    return  np.array([10, 10, 10E10]) * theta
...
# Define a likelihood function
def loglike_GeneralizedHalo(theta):
...
# It is a transformation from a space where variables are independently and uniformly distributed between 0 and 1 to the parameter space of interest. 
def prior_transform_GeneralizedHalo(theta):
    
    # Define mean and width of prior distributions.
    mu = np.array([5.,10.,4.,1.5,5E8])
    sigma = np.array([5.,10.,1.0,1.5,5E8])
